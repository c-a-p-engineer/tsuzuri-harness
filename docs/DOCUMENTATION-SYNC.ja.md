...
Documentation Sync用Workflowでは、少なくとも次を検証します。

- Capability一覧が存在する
- README / English・日本語PagesからCapability一覧へ辿れる
- 現在の主要Capabilityが一覧に含まれている
- Top-level `function/` Contractを追加・削除・Renameした場合、Capability一覧のEnglish / 日本語も更新されている

既存Contractの単純な内容変更については機械判定だけではPublic Impactを判断できないため、PR Templateで確認します。
